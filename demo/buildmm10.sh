...
#
# Downloads sequence for the mm10 version of M. musculus (mouse) from UCSC.
#
# Note that UCSC's mm10 build has two categories of compressed fasta files:
#
...
BWA_BUILD_EXE=./bwa
if [ ! -x "$BWA_BUILD_EXE" ] ; then
	if ! which bwa ; then
		echo "Could not find bwa in current directory or in PATH"
		exit 1
	else
		BWA_BUILD_EXE=`which bwa`
	fi
fi

CMD="${BWA_BUILD_EXE} index -a bwtsw -p mm10 mm10.fa"
echo Running $CMD
if $CMD ; then
